...
contract DebtProtocol is ERC20, Ownable {
    ...
    uint256 private _initialLiquidity = 32 * 10**9 * 10**DECIMALS;
    uint256 public unclaimedFees;
    address public _debtPancakeswapLPContract;
    mapping (address => uint256) private _balances;  
    ...
    uint256 hodlBoost = 100;                            	// Bonus for determining the user's Tier for those who never sold any token. 100 = 100% bonus of amount owned
    uint256 debtLag = 12 hours;                         	// After buying tokens on Pancakeswap, disable the debt timer for debtLag time
    uint256 minimumBuyAmount = 20 * 10**6 * 10**DECIMALS;       // Minimum amount to buy to activate the debtLag
    ...
    constructor(address master) public ERC20("Debt Protocol", "DEBT") { 
        ...
        _isExcluded[address(0x10ED43C718714eb63d5aA57B78B54704E256024E)] = true;    // Pancakeswap router excluded so it doesn't pay fees
...
//------------------------------------------------------------//
    ...
    function changeMinimumBuyAmount(uint256 _newMinimumBuyAmount)
        public
        onlyOwner
    {
    ...
    function setDebtPancakeswapLPContract(address _newDebtPancakeswapLPContract)
        public
        onlyOwner
    {
        _debtPancakeswapLPContract = _newDebtPancakeswapLPContract;
        excludeAccount(_newDebtPancakeswapLPContract);
    }
...
//------------------------------------------------------------//  
    ...
    function pendingDebt(address _user) public view returns (uint256) {
        ...
        if (userTier == maxTier || _balances[_user] == 0 || isExcluded(_user) || userInfo[_user].lastUpdate > now) {
            return 0;
        }
        else {
...
//------------------------------------------------------------//  
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 debtAmount) private {
        ...
        if (debtAmount >= minimumBuyAmount && oldBalance > 0) manageTier(recipient, newBalance, oldBalance, true);
